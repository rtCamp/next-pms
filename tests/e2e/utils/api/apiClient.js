 ...
 */
export const fetchWithRetry = async (requestContext, endpoint, fetchOptions, { retries = 3, backoffMs = 2000 } = {}) => {
  let attempt = 0;
  for (;;) {
    try {
...
export const isRowLockError = (err) =>
  /QueryTimeoutError|Lock wait timeout exceeded|being modified by another user/i.test(String(err?.message ?? ""));
 ...
 */
export const deleteWithLockRetry = async (deleteFn, { label = "document", retries = 4, backoffMs = 2000 } = {}) => {
  let attempt = 0;
  for (;;) {
    try {
      ...
      attempt += 1;
      if (attempt > retries) {
        throw new Error(`${label}: still locked after ${retries} retries. Last error: ${err.message}`);
      }
      const delay = backoffMs * 2 ** (attempt - 1);
      console.warn(`🔒 ${label} locked (attempt ${attempt}/${retries}). Retrying in ${delay}ms…`);
      await sleep(delay);
    }
  }
};
...
export const deleteDocument = async (doctype, name, role = "admin") => {
  ...
  const requestContext = await request.newContext({ baseURL, storageState: loadAuthState(role) });
  try {
    ...
    if (res.status() === 404 || /DoesNotExistError/.test(body)) return { deleted: true, alreadyGone: true };
...
export const getDocList = async (
  ...
  const requestContext = await request.newContext({ baseURL, storageState: loadAuthState(role) });
  try {
    const res = await fetchWithRetry(requestContext, `/api/method/frappe.client.get_list?${qs}`, { method: "GET" });
    if (!res.ok()) {
      console.warn(`⚠️ Could not list ${doctype}: ${res.status()} ${res.statusText()}`);
      return [];
    }
...
export const cancelDocument = async (doctype, name, role = "admin") => {
  ...
  try {
    ...
    const res = await fetchWithRetry(requestContext, "/api/method/frappe.client.cancel", {
      method: "POST",
      timeout: 120000,
      headers: { "Content-Type": "application/json" },
      data: { doctype, name },
    ...
    if (!res.ok()) {
      console.warn(`⚠️ Could not cancel ${doctype} ${name}: ${res.status()} ${res.statusText()}`);
    }
...
export const createDocument = async (doctype, data, role = "admin") => {
  const requestContext = await request.newContext({ baseURL, storageState: loadAuthState(role) });
  try {
    const res = await fetchWithRetry(requestContext, `/api/resource/${encodeURIComponent(doctype)}`, {
      method: "POST",
      timeout: 120000,
      headers: { "Content-Type": "application/json" },
      data,
    ...
    if (!res.ok()) {
      throw new Error(`Could not create ${doctype}: ${res.status()} ${res.statusText()}`);
    }
