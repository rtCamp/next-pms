...
| `.github/workflows/linter.yml` | **Linters** | `pre-commit` (*Frappe Linter*) — **semgrep findings ONLY** | **Only the `Sempgrep: frappe` hook is out-of-scope:** it runs `semgrep` security rules over the whole repo (~70+ pre-existing findings on `feat/redesign` in `next_pms/**/*.py`), which are backend security items with their own hardening cleanup. **The rest of this workflow is NOT ignorable** — see the warning below. |

> ⚠️ **Frappe Linter is only partially ignorable.** The same `pre-commit` job also runs **JS/TS Formatter (prettier)**, **ESLint React**, and **ESLint JS** against the PR's own changed files. These are NOT covered by `Frontend Build Test` (which only runs `vite`/`tsc` — no prettier, no eslint), so a green build does **not** mean the formatter/linter will pass. When the **Frappe Linter** check is red, open the log (`gh run view <run-id> --log-failed`) and split it: any `JS/TS Formatter` / `ESLint *` failure on a file **this PR changed** is **PR-caused → fix it and re-push**; only the `Sempgrep: frappe` Python findings are the pre-existing, out-of-scope part. Do not blanket-dismiss the whole check.
>
> **Prevent it:** there is no installed `.git/hooks/pre-commit`, so `git commit` skips pre-commit entirely (silently — not via `--no-verify`). Before committing any `.ts/.tsx/.js/.jsx`, run prettier on the changed files exactly as CI does (defaults → 80-col): inside `fm shell`, `cd apps/next_pms && npx --yes prettier@4.0.0-alpha.8 --write <changed files>`, then stage. (See the `next-pms-conventions` skill.)
