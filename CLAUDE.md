...
- **Per-cell rendering lives in its own file**, not a giant `switch(column.key)` block inside `index.tsx`. Individual visual components → **one per file** (`dot.tsx`, `stages-icon.tsx`, `date-cell.tsx`, `project-name-cell.tsx`, etc.) — don't aggregate into a single `cells.tsx`. **When the count reaches ~7+, group them under a `cells/` subfolder** with the dispatch as `cells/index.tsx`. Example final layout: `pages/projects/list/{index.tsx, columns.tsx, constants.ts, types.ts, fake-data.ts, cells/{index.tsx, dot.tsx, stages-icon.tsx, date-cell.tsx, project-name-cell.tsx, phase-cell.tsx, budget-progress-cell.tsx, employee-cell.tsx}}`. (PR #1220 round-3 correction: split out of `cells.tsx`; round-4 correction: group into `cells/` folder.)
