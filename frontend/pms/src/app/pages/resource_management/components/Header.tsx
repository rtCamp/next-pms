...
interface HeaderSectionProps {
  ...
  weekControl?: {
    isNeeded: boolean;
    handlePrevWeek: MouseEventHandler<HTMLButtonElement>;
    handleNextWeek: MouseEventHandler<HTMLButtonElement>;
  };
}

const ResourceHeaderSection = ({ filters, weekControl }: HeaderSectionProps) => {
  return (
    <Header className="flex items-center max-md:flex-col">
      ...
      {weekControl && weekControl.isNeeded && (
        <div id="date-filter" className="flex gap-x-2 max-md:p-1 max-md:w-full max-md:justify-between max-md:m-2 t">
          <Button title="prev" className="p-1 h-fit" variant="outline" onClick={weekControl.handlePrevWeek}>
            <ChevronLeft className="w-4 max-md:w-3 h-4 max-md:h-3" />
          </Button>
          <Button title="next" className="p-1 h-fit" variant="outline" onClick={weekControl.handleNextWeek}>
