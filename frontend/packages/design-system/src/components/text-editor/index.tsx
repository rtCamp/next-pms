...
const TextEditor = ({
  ...
  const handleChange = (
    ...
    const sanitizeHtml = (html: string) => {
      ...
      const isUnsafeStyle =
        /(expression|javascript:|url\s*\(|@import|behaviou?r|-moz-binding)/i;
      const isUnsafeProtocol = /^(?:javascript|vbscript|data(?!:image\/))/i;
      const sanitizeUrl = (url: string, allowImageData = false) => {
        ...
        if (
          allowImageData &&
          /^data:image\/[a-zA-Z0-9.+-]+;base64,[a-z0-9+/=]+$/i.test(trimmedUrl)
        ) {
          return trimmedUrl;
        }
        if (isUnsafeProtocol.test(normalizedUrl)) {
          return "";
        }
      ...
      Array.from(documentFragment.body.querySelectorAll("*")).forEach(
        (node) => {
          ...
          Array.from(element.attributes).forEach((attribute) => {
            ...
            if (attributeName === "style") {
              const sanitizedStyle = attribute.value
                .split(";")
                ...
                .map((styleRule) => {
                  ...
                  const property = styleRule
                    .slice(0, separatorIndex)
                    .trim()
                    .toLowerCase();
                  const propertyValue = styleRule
                    .slice(separatorIndex + 1)
                    .trim();

                  if (
                    !allowedStyleProperties.has(property) ||
                    isUnsafeStyle.test(propertyValue)
                  ) {
                    return "";
                  }
            ...
            if (
              ...
              !["_blank", "_parent", "_self", "_top"].includes(attribute.value)
            ) {
              element.removeAttribute(attribute.name);
            }
          });
    ...
    const formattedValue = sanitizeHtml(formatHtml(actualHtml));
