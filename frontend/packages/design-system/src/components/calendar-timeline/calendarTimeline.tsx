...
const CalendarTimeline = ({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="overflow-hidden rounded-lg border-t border-r border-outline-gray-1">
        <div className="grid" style={columnStyle}>
          {visibleDays.map((day) => (
            <div
              key={`weekday-${day.toISOString()}`}
              className="border-b border-l border-outline-gray-1 py-2 text-center text-base text-ink-gray-6"
            >
              {format(day, "EEE")}
            </div>
          ))}
          ...
          {visibleDays.map((day) => {
            ...
            return (
              <div
                key={day.toISOString()}
                className={cn(
                  "flex min-h-[140px] flex-col gap-0.5 border-b border-l border-outline-gray-1 py-px pl-0.5 pr-px",
