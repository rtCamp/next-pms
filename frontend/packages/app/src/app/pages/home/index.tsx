...
const Home = () => {
  return (
    <CustomViewWrapper
      label="Home"
      createFilter={createFilter({} as HomeState)}
    >
...
const HomeComponent = ({ viewData }: HomeComponentProps) => {
  ...
  useFrappeEventListener(`timesheet_info`, (payload) => {
    ...
    const dateExists = homeState.data.dates.some((item) =>
      item?.dates?.includes(payload.date),
    );
    if (!dateExists) {
      return;
    }
  ...
  const handleLoadMore = () => {
    ...
    dispatch({
      type: "SET_START",
      payload: homeState.start + homeState.pageLength,
    });
  };
        ...
        <Table className="[&_tr]:pr-3 relative">
          <TableHeader className="border-t-0 sticky top-0 w-full z-10 [&_tr]:border-b-0 [&_th]:py-0 group">
            <TableRow className="group-hover:bg-muted/50 ">
              ...
              <TableHead
                colSpan={7}
                className="h-fit divide-solid bg-slate-200 dark:bg-secondary "
              >
                <Typography
                  variant="p"
                  className="w-full flex justify-center items-center"
          ...
          <TableBody>
            {Object.entries(homeState.data?.data).length > 0 ? (
              Object.entries(
                homeState.data?.data as Record<string, DataItem>,
              ).map(([key, item]: [string, DataItem], index: number) => {
                const needToAddRef =
                  homeState.data.hasMore &&
                  index == Object.keys(homeState.data?.data).length - 2;
                return (
