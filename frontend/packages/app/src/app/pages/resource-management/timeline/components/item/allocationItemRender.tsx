...
const AllocationItemRender = ({
  ...
  const getTitle = (isNeedFullTitle = false) => {
    const title = `${
      resourceAllocation.project_name
        ? resourceAllocation.project_name + " "
        : ""
  ...
  itemProps = {
    ...
    style: {
      ...itemProps.style,
      padding: "1px",
      background: resourceAllocation.is_billable
        ? "rgba(147, 221, 137, 0.39)"
        : "#d7d77b26",
      borderRadius: "4px",
      border: "1px solid #d1d5db",
      borderWidth: 0,
      borderRightWidth:
        resourceAllocation.canDelete && itemContext.selected ? 3 : 0,
      overflow:
        dayDiff <= (resourceAllocation.isShowMonth ? 30 * 3 : 10)
          ? "hidden"
          : "visible",
    },
  };
      ...
      >
        <div
          className={mergeClassNames(
            "flex justify-start gap-[2px] h-full w-full",
          ...
          {itemContext.selected && resourceAllocation.canDelete && (
            <DeleteIcon
              ...
              resourceAllocationPermission={{
                delete: resourceAllocation.canDelete,
              }}
