...
export function ProfilePage({ displayName, email, image }: ProfilePageProps) {
  return (
    <div className="max-w-3xl">
      <div>
        <h2 className="text-lg font-semibold text-ink-gray-8">Profile</h2>
        <p className="mt-1 text-base text-ink-gray-6">
          Manage your profile and login information.
        </p>
      </div>
      <div className="mt-8 flex items-center gap-4 pb-8 pt-1.5">
        <Avatar label={displayName} image={image || undefined} size="3xl" />
        <div className="min-w-0">
          <p className="truncate text-lg font-semibold text-ink-gray-8">
            {displayName}
          </p>
          <p className="truncate text-base text-ink-gray-6">{email}</p>
        </div>
      </div>
      <section>
