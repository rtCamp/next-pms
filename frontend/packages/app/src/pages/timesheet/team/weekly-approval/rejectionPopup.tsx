...
import { Avatar, Button, ErrorMessage, Textarea } from "@rtcamp/frappe-ui-react";
...
const RejectionPopup = () => {
  const { employeeName, avatarUrl, dateRange, totalHours, handleRejectionSubmit, rejectionError } = useWeeklyApproval();
  ...
  return (
    <Dialog.Popup className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-100 z-101 bg-surface-modal rounded-xl shadow-xl flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 pt-5">
        <h1 className="text-2xl font-semibold text-ink-gray-8">Reason for timesheet rejection</h1>
      ...
      <div className="px-6 py-4 space-y-4">
        ...
        <div className="flex items-center justify-between bg-surface-gray-2 rounded-md p-2 text-base font-medium">
          ...
          <span className="text-ink-red-4">{floatToTime(totalHours, 2, 2)}</span>
        </div>
      ...
      <div className="px-6 py-4 space-y-1.5">
        ...
        <Button className="w-full" variant="solid" label="Reject timesheet" onClick={handleReject} />
      </div>
    </Dialog.Popup>
  );
};
