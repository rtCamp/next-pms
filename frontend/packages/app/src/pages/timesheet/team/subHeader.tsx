...
export function SubHeader() {
  ...
  const approvalStatus = useTeamTimesheet(({ state }) => state.filters.approvalStatus);
  const reportsTo = useTeamTimesheet(({ state }) => state.filters.reportsTo);
  const compositeFilters = useTeamTimesheet(({ state }) => state.compositeFilters);

  const handleSearchChange = useTeamTimesheet(({ actions }) => actions.handleSearchChange);
  const handleApprovalStatusChange = useTeamTimesheet(({ actions }) => actions.handleApprovalStatusChange);
  const handleReportsToChange = useTeamTimesheet(({ actions }) => actions.handleReportsToChange);
  const handleCompositeFilterChange = useTeamTimesheet(({ actions }) => actions.handleCompositeFilterChange);
  const handleClearAllFilters = useTeamTimesheet(({ actions }) => actions.handleClearAllFilters);
  ...
  const externalFilterCount = (search !== "" ? 1 : 0) + (approvalStatus ? 1 : 0) + (reportsTo ? 1 : 0);
