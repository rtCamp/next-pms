...
import type {
  ProjectFilterArgs,
  ProjectWeeksResponse,
  ProjectWeekSummary,
} from "./types";

type UseProjectTimesheetWeeksOptions = {
  date: string;
  maxWeek: number;
  filterArgs: ProjectFilterArgs;
};

type UseProjectTimesheetWeeksResult = {
...
const QUERY_SIGNATURE_PREFIX = "project-timesheet-weeks:";

export function useProjectTimesheetWeeks({
  date,
  maxWeek,
  filterArgs,
}: UseProjectTimesheetWeeksOptions): UseProjectTimesheetWeeksResult {
  ...
  const getKey = useCallback(
    (
      pageIndex: number,
      previousPageData: ProjectWeeksResponse | null,
  ...
  const {
    ...
    mutate,
    error,
  } = usePagination<ProjectWeeksResponse>(
    "next_pms.timesheet.api.project.get_project_timesheet_weeks",
    getKey,
    {
      max_week: maxWeek,
      ...filterArgs,
