...
export function Tracking() {
  ...
  return (
    <TrackingProvider projectId={projectId}>
      <TrackingContent projectId={projectId} />
    </TrackingProvider>
  );
}

function TrackingContent({ projectId }: { projectId: string }) {
    ...
    <div className="flex flex-col gap-6">
      <div className="flex gap-3">
        ...
        <KnowledgePoint
          ...
          value={formatter.format(tracking.projectProfit)}
        />
        <KnowledgePoint
          title="Projected profit margin"
          value={`${tracking.projectedProfitMargin.toFixed(2)}%`}
        />
      </div>
